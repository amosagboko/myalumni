...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex align-items-center">
            ...
            <span class="refresh-indicator" id="connectionStatus">Connecting...</span>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Subscribe to the election channel
    const channel = pusher.subscribe('election.{{ $election->id }}');
    
    // Listen for results updates
    channel.bind('results.updated', function(data) {
        updateStats(data.results);
        updateOfficeResults(data.results);
        lastUpdate = new Date();
    ...
    setInterval(() => {
        ...
        if (diff > 0) {
            document.getElementById('connectionStatus').textContent = `Last updated ${diff} seconds ago`;
        }
    }, 1000);
});
</script>
