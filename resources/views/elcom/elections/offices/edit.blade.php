        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header bg-white">
                    <h3 class="card-title">Edit Office - {{ $office->title }}</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('elcom.election-offices.update', [$election, $office]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                ...
                                name="description" 
                                rows="3" 
                                required>{{ old('description', $office->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" 
                                ...
                                id="max_candidates" 
                                name="max_candidates" 
                                value="{{ old('max_candidates', $office->max_candidates) }}" 
                                min="1" 
                                required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" 
                                ...
                                id="max_terms" 
                                name="max_terms" 
                                value="{{ old('max_terms', $office->max_terms) }}" 
                                min="1" 
                                required>
                            @error('max_terms')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('fee_type_id') is-invalid @enderror" 
                                ...
                                @foreach($feeTypes as $feeType)
                                    <option value="{{ $feeType->id }}" {{ old('fee_type_id', $office->fee_type_id) == $feeType->id ? 'selected' : '' }}>
