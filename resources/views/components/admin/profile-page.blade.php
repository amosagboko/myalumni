@props([
    ...
    'photoHint' => 'Upload a photo for your account.',
    'embedded' => false,
])
...
@php
    $dashboardRoute = $dashboardRoute ?? (
        auth()->user()?->hasRole('administrator')
            ...
            : (auth()->user()?->hasRole('alumni-relations-officer')
                ? route('alumni-relations-officer.home')
                : (auth()->user()?->hasRole('elcom-chairman')
                    ? route('elcom-chairman.home')
                    : (auth()->user()?->hasRole('student-affairs')
                        ? route('student-affairs.home')
                        : (auth()->user()?->hasRole('academic-affairs')
                            ? route('academic-affairs.home')
                            : url('/')))))
    );
@endphp
...
@if ($embedded)
    <div class="admin-surface admin-profile-page">
        <div class="ads-profile-shell">
@else
    <div class="main-content admin-surface admin-profile-page" style="padding-right: 1.25rem;">
...
@endif
                    ...
                    <div class="ads-section">
                        <div class="ads-section-card">
                            ...
                            <p class="text-muted small mb-3">{{ $photoHint }}</p>
